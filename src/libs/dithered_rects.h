...
int loop_count = 0;
uint8_t transition_second_color = 0;
	...
	if(transition_loops == 0 || loop_count < transition_loops) {
		if(increasing){
			switch(current_dither){
				...
				case DITHER_80_PERCENT: 
					current_dither = DITHER_90_PERCENT;
					if(transition_second_color == GColorClear.argb)
						loop_count++;
					break;
...
void draw_transitioning_rect(GContext *ctx, GRect bounds, GColor first_color, GColor second_color){
	transition_second_color = second_color.argb;
